...
import { z } from "zod";
import { insertRatingSchema, insertPoliticianSchema, insertPromiseSchema, insertBillSchema, insertVotingRecordSchema } from "@shared/schema";
...
export async function registerRoutes(app: Express): Promise<Server> {
    ...
    try {
      ...
      const politiciansWithRatings = await Promise.all(
        politicians.map(async (politician) => {
          const rating = await storage.getPoliticianAverageRating(politician.id);
          return {
            ...politician,
  ...
  app.get("/api/politicians/:id", async (req, res) => {
    try {
      ...
      const ratings = await storage.getRatingsByPoliticianId(id);
      ...
      const fulfillmentStats = {
        ...
        inProgress: promises.filter(p => p.status === "InProgress").length,
        unfulfilled: promises.filter(p => p.status === "Unfulfilled").length,
    ...
    try {
      ...
      const ratingsWithPolitician = await Promise.all(
        pendingRatings.map(async (rating) => {
          const politician = await storage.getPolitician(rating.politicianId);
          return {
            ...rating,
            politician
          };
  ...
  app.get("/api/comparison", async (req, res) => {
    try {
      const ids = (req.query.ids as string || "").split(",").map(id => parseInt(id)).filter(id => !isNaN(id));
      ...
      validData.forEach(data => {
        data?.votingRecords.forEach(record => {
          allBillIds.add(record.billId);
        });
      });
      
      const commonBills = Array.from(allBillIds).map(billId => {
        // Get the bill
        const bill = validData[0]?.votingRecords.find(r => r.billId === billId)?.bill;
        
        // Get votes for each politician
        const votes = validData.map(data => {
          const record = data?.votingRecords.find(r => r.billId === billId);
          return {
  ...
  app.get("/api/politicians/:id/voting-records", async (req, res) => {
    try {
      ...
      const billsMap: Record<number, any> = {};
      
      for (const record of votingRecords) {
        const bill = await storage.getBill(record.billId);
        if (bill) {
          billsMap[record.billId] = bill;
        }
      }
